//! LDPC / sync tables for FT8 and FT4.
//!
...
/// Parity generator matrix, bit-packed MSB first.
pub const LDPC_GENERATOR: [[u8; 12]; 83] = [
    [
...
/// Each row lists the codeword bits (1-origin) in one parity check.
...
/// Each row lists the three parity checks (1-origin) a codeword bit joins.
...
/// Live entries per row of `LDPC_NM` (rows are padded to 7).
...
mod tests {
    ...
    /// Byte sums computed from ft8_lib's constants.c at port time. A
    /// mis-transcribed digit changes one of these; nothing else would.
    #[test]
    fn table_checksums() {
        assert_eq!(
            LDPC_GENERATOR
        ...
        assert_eq!(
            LDPC_NM.iter().flatten().map(|&v| v as u32).sum::<u32>(),
            45675
        );
        assert_eq!(
            LDPC_MN.iter().flatten().map(|&v| v as u32).sum::<u32>(),
            21806
        );
        assert_eq!(LDPC_NUM_ROWS.iter().map(|&v| v as u32).sum::<u32>(), 522);
        assert_eq!(
            FT8_COSTAS_PATTERN.iter().map(|&v| v as u32).sum::<u32>(),
            21
        );
        // Every Nm row must list exactly as many bits as Num_rows claims.
        for (r, &n) in LDPC_NUM_ROWS.iter().enumerate() {
            assert!(n as usize <= 7);
            for c in (n as usize)..7 {
                assert_eq!(LDPC_NM[r][c], 0, "row {r} pad");
            }
            for c in 0..(n as usize) {
                assert!(LDPC_NM[r][c] as usize <= LDPC_N);
            }
        }
        // Mn is the transpose of Nm: each bit claims 3 checks that must claim it back.
        for bit in 0..LDPC_N {
            for &chk in &LDPC_MN[bit] {
                if chk == 0 {
                    continue;
                }
                let row = &LDPC_NM[chk as usize - 1];
                assert!(
                    row.iter().any(|&b| b as usize == bit + 1),
                    "bit {bit} claims check {chk}, which does not claim it back"
                );
            }
        }
    }
